    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="rounded-3xl bg-white p-8 shadow-lg text-center">
          <p className="text-slate-700">Memuat...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        <div className="rounded-3xl bg-white p-8 shadow-lg">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link href="/dashboard" className="rounded-full border border-slate-300 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-900 transition hover:bg-slate-100">
              Kembali
            </Link>
          </div>
                  ...
                  <tr key={user.id} className="border-b border-slate-200 hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${
                        ...
                        user.role === 'owner' || user.role === 'landlord' ? 'bg-emerald-100 text-emerald-700' :
